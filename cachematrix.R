...
makeCacheMatrix <- function(x = matrix()) {
  ...
  set <- function(y) { #the matrix can be changed using the set object and a matrix arguement y
    x <<- y       
  ...
  setinverse <- function(inverse) i <<- inverse #inverse is set when called from cacheSolve function
...
cacheSolve <- function(x, ...) { #the argument x is a special matrix that can store its inverse
  i <- x$getinverse()
  if(!anyNA(i)) { #if the inverse has been calculated already then notify the user and print the inverse
